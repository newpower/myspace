...
(function($){
	...
	var settings, callback, loadedWidth, loadedHeight, interfaceHeight, interfaceWidth, index, related, ssTimeout, $slideshow, $window, $close, $next, $prev, $current, $title, $modal, $wrap, $loadingOverlay, $loadingGraphic, $overlay, $modalContent, $loaded, $borderTopCenter, $borderMiddleLeft, $borderMiddleRight, $borderBottomCenter, $fullPhotoLink;
	...
	*/
	$.fn.colorbox.init = function(){
		...
		$modalContent.append(
			...
			$fullPhotoLink = $('<a id="cboxfullPhotoLink"></a>'),
...
// TODO надо сделать чтобы имя параметра из даты передавалась в вызове и сохранялось в settings
		var fullPhotoLink = $(related[index]).data('bigPhoto');
		if (fullPhotoLink) {
			$fullPhotoLink
				.text('оригинальный размер')
				.attr({
					href: fullPhotoLink.href,
					target: '_blank'
				})
		} else {
			$fullPhotoLink
