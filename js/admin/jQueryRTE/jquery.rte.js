...
/*var lwRTE_resizer = function(a) {
    this.drag = false;
    this.rte_zone = jQuery(a).parents(".rte-zone")
};
lwRTE_resizer.mousedown = function(b, a) {
    b.drag = true;
    b.event = (typeof(a) == "undefined") ? window.event: a;
    b.rte_obj = jQuery(".rte-resizer", b.rte_zone).prev().eq(0);
...
var lwRTE = function(a, b) {
    ...
    this.width = b.width || jQuery(a).width() || "99%";
//	посмотрим что из этого выйдет, до этого размер прописывался не правильный, появлялась прокрутка
    this.height = b.height || jQuery(a).height() || 350;
    this.iframe = null;
    ...
    jQuery.extend(this.controls.rte, b.controls_rte || {});
    jQuery.extend(this.controls.html, b.controls_html || {});
    jQuery.extend(this.css, b.css || {});
    if (document.designMode || document.contentEditable) {
        jQuery(a).wrap(jQuery("<div></div>").addClass("rte-zone").width(this.width));
        //jQuery('<div class="rte-resizer"><a href="#"></a></div>').insertAfter(a);
        //var c = new lwRTE_resizer(a);
        jQuery(".rte-resizer a", jQuery(a).parents(".rte-zone")).mousedown(function(d) {
...
lwRTE.prototype.get_toolbar = function() {
    var a = (this.iframe) ? jQuery(this.iframe) : jQuery(this.textarea);
...
lwRTE.prototype.activate_toolbar = function(c, a) {
    ...
    jQuery(c).before(jQuery(a).clone(true))
};
lwRTE.prototype.enable_design_mode = function() {
    ...
    if (jQuery(a.textarea).attr("class")) {
        a.iframe.className = jQuery(a.textarea).attr("class")
    }
    if (jQuery(a.textarea).attr("id")) {
        a.iframe.id = jQuery(a.textarea).attr("id")
    }
    if (jQuery(a.textarea).attr("name")) {
        a.iframe.title = jQuery(a.textarea).attr("name")
    }
    var f = jQuery(a.textarea).val();
    jQuery(a.textarea).hide().after(a.iframe).remove();
    a.textarea = null;
    var c = "";
    for (var b in a.css) {
    ...
    try {
        ...
        jQuery(a.iframe_doc).focus(function() {
    ...
    jQuery(a.iframe).parents("form").submit(function() {
        a.disable_design_mode(true)
    });
    jQuery(a.iframe_doc).mouseup(function(e) {
    ...
    jQuery(a.iframe_doc).blur(function(e) {
        if (a.iframe_doc.selection) {
            a.range = a.iframe_doc.selection.createRange()
        }
    });
    jQuery(a.iframe_doc).keyup(function(e) {
...
lwRTE.prototype.disable_design_mode = function(b) {
    var a = this;
    a.textarea = (b) ? jQuery('<input type="hidden" />').get(0) : jQuery("<textarea></textarea>").width("100%").height(a.height).get(0);
    ...
    jQuery(a.textarea).val(jQuery("body", a.iframe_doc).html());
    jQuery(a.iframe).before(a.textarea);
    if (!a.toolbars.html) {
        a.toolbars.html = a.create_toolbar(a.controls.html)
    }
    if (b != true) {
        jQuery(a.iframe_doc).remove();
        jQuery(a.iframe).remove();
...
lwRTE.prototype.toolbar_click = function(f, d) {
    ...
    if (b) {
        ...
        //try {
            b.apply(this, a)
        //} catch(c) {
            //console.log("ERROR: ",c,", b: ",b,", a: ",a, ", this",this);
        //}
    } else {
...
lwRTE.prototype.create_toolbar = function(d) {
    var c = this;
    var b = jQuery("<div></div>").addClass("rte-toolbar").width("100%").append(jQuery("<ul></ul>")).append(jQuery("<div></div>").addClass("clear"));
    var h, a;
    for (var f in d) {
        if (d[f].separator) {
            a = jQuery("<li></li>").addClass("separator")
        } else {
            if (d[f].init) {
                try {
            ...
            if (d[f].select) {
                h = jQuery(d[f].select).change(function(i) {
                    c.event = i;
                    c.toolbar_click(this, d[this.className]);
                    return false
                })
            } else {
                h = jQuery("<a href='#'></a>").attr("title", (d[f].hint) ? d[f].hint: f).attr("rel", f).click(function(i) {
                    c.event = i;
                    c.toolbar_click(this, d[this.rel]);
                    return false
                })
            }
            a = jQuery("<li></li>").append(h.addClass(f))
        }
        jQuery("ul", b).append(a)
    }
    jQuery(".enable", b).click(function() {
        c.enable_design_mode();
        return false
    });
    jQuery(".disable", b).click(function() {
...
lwRTE.prototype.create_panel = function(h, c) {
    ...
    jQuery(".rte-panel", e).remove();
    var f, b;
    var d = i.event.pageX;
    var g = i.event.pageY;
    var a = jQuery("<div></div>").hide().addClass("rte-panel").css({
        left: d,
        top: g
    });
    jQuery("<div></div>").addClass("rte-panel-title").html(h).append(jQuery("<a class='close' href='#'>X</a>").click(function() {
        a.remove();
        return false
    })).mousedown(function() {
        f = true;
...
lwRTE.prototype.set_content = function(a) { (this.iframe) ? jQuery("body", this.iframe_doc).html(a) : jQuery(this.textarea).val(a)
};
